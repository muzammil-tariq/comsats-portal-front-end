...
import { loginFaculty, loginStudent } from "src/features/auth/authAction";
...
export const FacultyLogin = () => {
  ...
  const { isProcessingRequest, message, loginSuccessful } = useSelector(authSelector);
  const FacultyLogin = async (email: string, password: string) => {
    try {
      // pass the function in dispatch
      await dispatch(loginFaculty(email, password));
      console.log(localStorage.getItem("facultyId"));
      console.log(localStorage.getItem("token"));
      // console.log(localStorage.getItem('facultyId'));
      if (localStorage.getItem("token") && localStorage.getItem("facultyId")) {
